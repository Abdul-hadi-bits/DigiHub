...
class FirebaseChatCore {
  ...
  Future<types.Room> getRoom({required String roomId}) async {
    try {
      final fu = firebaseUser!;
      final docSnapshot = await getFirebaseFirestore()
          .collection(config.roomsCollectionName)
          .doc(roomId)
          .get();
  ...
  /// custom data.
  /// If room already exits it will be returned, else it will create a new room and returns
  /// a room object with only the room id, not the actual room that is going to be created
  /// in firebase.
  ...
  Stream<types.Room> room(String roomId) {
    ...
    final stream = getFirebaseFirestore()
        .collection(config.roomsCollectionName)
        .doc(roomId)
        .snapshots();
    stream.asyncMap((event) => null);
  ...
  Future<void> sendMessage(dynamic partialMessage, String roomId) async {
    ...
    if (partialMessage is types.PartialVideo) {
  ...
  Future<void> updateLastMessage({
    ...
    if (updateOnlyText == true) {
      messageMap.removeWhere((key, value) => key == 'author' || key == 'id');
      messageMap['authorId'] = firebaseUser!.uid;
      messageMap['createdAt'] =
          Timestamp.fromMillisecondsSinceEpoch(messageMap['createdAt']);
      messageMap['updatedAt'] = FieldValue.serverTimestamp();
    ...
    messageMap['createdAt'] =
        Timestamp.fromMillisecondsSinceEpoch(messageMap['createdAt']);
    messageMap['updatedAt'] =
        Timestamp.fromMillisecondsSinceEpoch(messageMap['updatedAt']);
    await getFirebaseFirestore()
        .collection(config.roomsCollectionName)
        .doc(roomId)
        .update({
  ...
  void updateUserOnlineStatus({
    ...
    try {
      ...
      roomMap.removeWhere((key, value) => key != 'userStatus');

      final userStatusMap = roomMap['userStatus'] as Map<String, dynamic>;
      userStatusMap.addAll({firebaseUser!.uid: isOnline});
      roomMap['userStatus'] = userStatusMap;
